...
func main() {
	...
	for i := 0; i < n; i++ {
		for j := 0; j < w+1; j++ {
			for l := 1; l < k+1; l++ {
				if a[i] <= j {
					dp[i+1][j][l] = max(dp[i][j][l], dp[i][j-a[i]][l-1]+b[i])
				} else {
					dp[i+1][j][l] = dp[i][j][l]
				}
				// fmt.Printf("dp[%d][%d][%d]=%d", i+1, j, l, dp[i+1][j][l])
			}
			// fmt.Println("")
		}
	}
	fmt.Println(dp[n][w][k])
}
...
// stringをスペース区切りの配列に変換
func getStringSliceSpace(s string) []string {
