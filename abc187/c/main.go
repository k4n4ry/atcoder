...
func main() {
	...
	for i := 0; i < n; i++ {
		...
		if strings.HasPrefix(s, "!") {
			ess = append(ess, strings.TrimLeft(s, "!"))
		} else {
			ss = append(ss, s)
		}
	}
	sort.SliceStable(ess, func(i, j int) bool {
		return ess[i] < ess[j]
	})
	...
	for i := 0; i < len(ss); i++ {
		tgt := ss[i]
		idx := sort.Search(len(ess), func(k int) bool { return ess[k] >= tgt })
		if idx < len(ess) && ess[idx] == tgt {
			ans = tgt
			break
		} else {
			continue
