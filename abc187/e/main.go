...
func main() {
	...
	for i := 0; i < n-1; i++ {
		...
		b := sc.readInt() - 1
		abs[i] = ab{a, b}
		graph[a].cld = append(graph[a].cld, b)
		graph[b].cld = append(graph[b].cld, a)
	}
	// graphの子ノードをsorting(sort.Searchを使用するため)
	for i := 0; i < len(graph); i++ {
		if len(graph[i].cld) > 1 {
			sort.SliceStable(graph[i].cld, func(ii, jj int) bool {
				return graph[i].cld[ii] < graph[i].cld[jj]
			})
		}
	}
	var sk []int
	sk = append(sk, 0)
	rm := func(tgt int, s *[]int) {
		var idx int
		for i := range *s {
			if (*s)[i] == tgt {
				idx = i
	...
	// graph[0]を根として、単方向グラフに変換する
	// 自身の子ノードの子ノードリストから自身のインデックスを削除していく
	for len(sk) > 0 {
		tgt := sk[0]
		sk = sk[1:]
		for i := 0; i < len(graph[tgt].cld); i++ {
			rm(tgt, &graph[graph[tgt].cld[i]].cld)
			sk = append(sk, graph[tgt].cld[i])
		}
	}
	q := sc.readInt()
	search := func(n int, ns []int) bool {
		if len(ns) == 0 {
			return false
		}
		if len(ns) == 1 {
		...
		idx := sort.Search(len(ns), func(k int) bool { return ns[k] >= n })
		if idx < len(ns) && ns[idx] == n {
			return true
		}
		return false
	}
	// クエリをreadし、ノードに対してvalをupdateする
	for i := 0; i < q; i++ {
		t := sc.readInt()
		e := sc.readInt() - 1
		x := sc.readInt()
		var f, to int
		if t == 1 {
			f, to = abs[e].a, abs[e].b
		} else {
			f, to = abs[e].b, abs[e].a
		}
		if search(to, graph[f].cld) {
			graph[to].val += -x
			graph[0].val += x
		} else {
			graph[f].val += x
	...
	// 根から累積和で下ろして最終的な値を取得する
	var stk []nd
	stk = append(stk, graph[0])
	for len(stk) != 0 {
		tgt := stk[0]
		stk = stk[1:]
		for i := 0; i < len(tgt.cld); i++ {
			graph[tgt.cld[i]].val += tgt.val
			stk = append(stk, graph[tgt.cld[i]])
		}
	}
	for i := 0; i < len(graph); i++ {
		fmt.Println(graph[i].val)
	}
