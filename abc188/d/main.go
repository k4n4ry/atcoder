...
func main() {
	...
	n := sc.readInt()
	co := sc.readInt()
	var mp = make(map[int][]ab)
	for i := 0; i < n; i++ {
		a := sc.readInt()
		b := sc.readInt()
		c := sc.readInt()
		mp[a] = append(mp[a], ab{a, true, c})
		mp[b+1] = append(mp[b+1], ab{b + 1, false, c})
	}
	var abss [][]ab
	for _, v := range mp {
		abss = append(abss, v)
	}
	sort.SliceStable(abss, func(i, j int) bool {
		return abss[i][0].t < abss[j][0].t
	})
	var sum, fee, ans int
	var zen int
	for i := 0; i < len(abss); i++ {
		ab := abss[i]
		for j := range ab {
			if ab[j].ud {
				sum += ab[j].c
