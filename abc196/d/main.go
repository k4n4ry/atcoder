...
func main() {
	sc := newScanner()
	h, w, a, b := sc.readInt(), sc.readInt(), sc.readInt(), sc.readInt()
	var m = make([][]bool, h)
	for i := 0; i < h; i++ {
		var tmp = make([]bool, w)
		m[i] = tmp
...
func dfs(m [][]bool, a, b int, pt pt) {
	if isFull(m) {
		ans++
		return
	}
	if b > 0 {
		if ok := getHanjoPt(m, pt); ok {
			mcp := cop(m)
			mcp[pt.h][pt.w] = true
			dfs(mcp, a, b-1, nextPt(mcp))
		}
	}
	if a > 0 {
		if ok := getZenjoMigi(m, pt); ok {
			mcp := cop(m)
			mcp[pt.h][pt.w] = true
			mcp[pt.h][pt.w+1] = true
			dfs(mcp, a-1, b, nextPt(mcp))
		}
		if ok := getZenjoShita(m, pt); ok {
			mcp := cop(m)
			mcp[pt.h][pt.w] = true
			mcp[pt.h+1][pt.w] = true
			dfs(mcp, a-1, b, nextPt(mcp))
		}
	}
}

func nextPt(m [][]bool) pt {
	for i := 0; i < len(m); i++ {
		for j := 0; j < len(m[i]); j++ {
			if !m[i][j] {
				return pt{i, j}
...
func cop(m [][]bool) [][]bool {
	var ret = make([][]bool, len(m))
	for i := 0; i < len(m); i++ {
		var rettmp = make([]bool, len(m[i]))
		for j := 0; j < len(m[i]); j++ {
			rettmp[j] = m[i][j]
		}
		ret[i] = rettmp
...
func getHanjoPt(m [][]bool, pt pt) bool {
	if !m[pt.h][pt.w] {
		return true
	}
	return false
...
func getZenjoMigi(m [][]bool, pt pt) bool {
	if !m[pt.h][pt.w] && len(m[pt.h])-1 != pt.w && !m[pt.h][pt.w+1] {
		return true
	}
	return false
}

func getZenjoShita(m [][]bool, pt pt) bool {
	if !m[pt.h][pt.w] && len(m)-1 != pt.h && !m[pt.h+1][pt.w] {
		return true
	}
	return false
}

func isFull(m [][]bool) bool {
	for i := 0; i < len(m); i++ {
		for j := 0; j < len(m[i]); j++ {
			if !m[i][j] {
				return false
