...
func main() {
	sc := newScanner()
	n, d, h := sc.readInt(), sc.readInt(), sc.readInt()
	type intr struct {
		d int
		h int
	}
	var intrs = make([]intr, n)
	for i := 0; i < n; i++ {
		d := sc.readInt()
		h := sc.readInt()
		intrs[i] = intr{d, h}
	}
	var min float64 = float64(math.MaxInt16)
	for i := 0; i < len(intrs); i++ {
		intr := intrs[i]
		var kat float64 = float64((h - intr.h)) / float64((d - intr.d))
		if min > kat {
			min = kat
		}
	}
	var ans = float64(h) - (min * float64(d))
	if ans < 0 {
		ans = 0
	}
	fmt.Println(ans)
